...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      const songs = _.chunk(Searched.tracks, 10);
      const Pages = songs.map((songz) => {
        const MappedSongs = songz.map(
          (s) =>
            `${s.index + 1}. [${s.title}](${
              s.uri
      ...
      const w = (a) => new Promise((r) => setInterval(r, a));
      await w(500);
      ...
      let er = false;
      const SongID = await message.channel
        // skipcq
